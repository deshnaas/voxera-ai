...
""".strip()
    ...
    def _format_recall_response(self) -> str:
        """
        Build a concise factual response directly from stored context.
        This prevents the LLM from dropping details during recall.
        """
        ctx = self.patient_context
        pieces = []

        if ctx.get("age") is not None:
            pieces.append(f"you're {ctx['age']}")

        symptoms = list(ctx.get("symptoms") or [])
        associated = list(ctx.get("associated_symptoms") or [])

        all_symptoms = []
        for item in symptoms + associated:
            if item.lower() not in {
                x.lower() for x in all_symptoms
            }:
        ...
        if ctx.get("duration"):
            pieces.append(
                f"they've been happening for {ctx['duration']}"
            )

        if ctx.get("severity"):
            pieces.append(
                f"you described the severity as {ctx['severity']}"
            )

        if ctx.get("current_concern"):
            concern = str(ctx["current_concern"])
            if concern.lower() not in {
                x.lower() for x in all_symptoms
            }:
                pieces.append(
                    f"your current concern is {concern}"
                )

        # If structured extraction did not contain enough detail,
        # use recent user turns as a factual fallback.
        if not pieces and self.history:
        ...
        if not pieces:
            return "I don't have enough previous information to summarize yet."

        if len(pieces) == 1:
            return "You told me " + pieces[0] + "."

        return "You told me " + ", and ".join(pieces) + "."
