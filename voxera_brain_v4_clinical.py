...
- Treat the PATIENT CONTEXT and recent conversation as authoritative for
  information already provided.
- Use known facts explicitly when they matter.
- Never ask again for information that is already known.
- Understand short replies in context.
- Never replace a known fact with a generic question.
- Do not invent missing clinical details just to make the conversation flow.
...
- Do not repeatedly ask "how are you feeling now?"
- Do not restart the conversation after every answer.
- Do not ask about a fact that is already explicitly known.
- If enough information is available, give a useful next step instead
  of asking another unnecessary question.
- Continue the current clinical thread until the patient changes it.
...
- If medication or treatment is discussed, keep advice conservative
  and account for known allergies, age, and relevant context when
  available.
- Do not suggest a treatment merely to fill space; only suggest it when
  it is reasonably appropriate to the information provided.
...
class VoxeraBrain:
    ...
    def _build_context_text(
        ...
        if ctx.get("last_question"):
            parts.append(
                f"Question Voxera most recently asked: {ctx['last_question']}"
            )
        ...
        if self.patient_turns:
            recent_patient_turns = self.patient_turns[-6:]
            parts.append(
                "VERBATIM RECENT PATIENT TURNS:\n"
                + "\n".join(
                    f"- {turn}" for turn in recent_patient_turns
                )
            )
...
Do not repeat the most recent question unless the patient explicitly
...
""".strip()
    ...
    def _save_turn(
        ...
        # Structured context can be imperfect. Keep the actual patient
        # utterances as the source of truth for conversation recall.
        self.patient_turns.append(user_message.strip())
    ...
    # ========================================================
    # RECALL / MEMORY QUESTIONS
    # ========================================================

    def _is_recall_request(self, text: str) -> bool:
        """
        Detect requests whose primary purpose is to recall what the
        patient previously told Voxera.

        This is intentionally a small conversational detector rather
        than a medical keyword list.
        """
        normalized = re.sub(
            r"[^a-z0-9\s]",
            " ",
    ...
    def _format_recall_response(self) -> str:
        """Recall what the patient actually told Voxera."""
        turns = [
            t.strip()
            for t in self.patient_turns
            if t and t.strip()
        ]

        # The current recall question is itself a stored patient turn.
        if turns and self._is_recall_request(turns[-1]):
            turns = turns[:-1]

        if not turns:
            return "You haven't told me much yet."

        # For short conversations, preserve all actual patient facts.
        unique = []
        for turn in turns:
            if turn not in unique:
                unique.append(turn)

        if len(unique) <= 4:
            return "You told me: " + " ".join(unique)

        # For longer calls, give a compact state summary and preserve the
        # most recent exact patient information.
        ctx = self.patient_context
        facts = []

        if ctx.get("age") is not None:
            facts.append(f"you're {ctx['age']}")

        symptoms = []
        for item in (ctx.get("symptoms") or []) + (ctx.get("associated_symptoms") or []):
            text = str(item).strip()
            if text and text.lower() not in {x.lower() for x in symptoms}:
                symptoms.append(text)

        if symptoms:
            facts.append("you mentioned " + ", ".join(symptoms))

        if ctx.get("duration"):
            facts.append(f"they've been happening for {ctx['duration']}")

        if facts:
            return "You told me " + ", ".join(facts) + "."

        return "You told me: " + " ".join(unique[-4:])

    # ========================================================
    # CHAT
    # ========================================================

    def _normalize_clinical_response(self, response_text: str) -> str:
        """Conservative cleanup for unsupported clinical assumptions."""
        text = str(response_text).strip()
        lower = text.lower()

        asthma_phrases = (
            "since your symptoms seem to be related to asthma",
            "your symptoms are related to asthma",
            "you are experiencing asthma",
            "you have asthma",
        )
        if any(p in lower for p in asthma_phrases):
            return (
                "Wheezing can happen with asthma, but it can also have other causes, "
                "so I can't confirm asthma from this conversation alone."
            )

        if "use a rescue inhaler" in lower and "prescribed" not in lower:
            return (
                "If you have a prescribed rescue inhaler, use it only as directed by "
                "your clinician. If you don't have one, don't use someone else's medication."
            )

        if "911" in lower:
            text = text.replace("911", "your local emergency service")
        return text
    ...
    def chat(
        ...
        # Recall requests are answered directly from persistent state.
        # This is more reliable than asking the small LLM to reconstruct
        # a multi-turn summary from a short context window.
        if self._is_recall_request(user_message):
            response_text = self._format_recall_response()
        ...
        response_text = self._normalize_clinical_response(response_text)
        ...
        # Keep the latest spoken question in state. This gives the next
        # model turn an explicit signal about what has already been asked.
        if "?" in response_text:
            question = response_text.rsplit("?", 1)[0].strip()
            if question:
                self.patient_context["last_question"] = (
                    question.split(".")[-1].strip() + "?"
                )
        else:
            self.patient_context["last_question"] = None
