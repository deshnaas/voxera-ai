...
# BARGE-IN STATE
# ============================================================
# These variables affect ONLY Priya playback interruption.
# The normal patient VAD below is left unchanged.
pending_barge_audio = None
barge_start_threshold = 0.0060

# WebRTC AEC / VAD for barge-in only.
# This removes Priya's speaker echo before interruption detection.
BARGE_AEC_DELAY_MS = 60
BARGE_SPEECH_PROBABILITY = 0.72
BARGE_MIN_SPEECH_SECONDS = 0.22
BARGE_FRAME_SAMPLES = SAMPLE_RATE // 100  # 10 ms
...
# ============================================================
# BARGE-IN DETECTION — PRIYA SPEAKING ONLY
# ============================================================

BARGE_MIN_SPEECH_SECONDS = 0.20
BARGE_PRE_ROLL_SECONDS = 0.40
BARGE_ECHO_CORRELATION = 0.84
...
def _looks_like_priya_echo(mic_chunk, playback_audio, playback_rate, playback_position):
    """
    Decide whether a microphone chunk is mostly Priya leaking back
    through the speaker/microphone path.

    We compare the live microphone chunk against nearby portions of
    the exact WAV being played.  A high correlation means the mic is
    hearing Priya rather than an independent patient utterance.
    """

    if playback_audio is None or len(playback_audio) == 0:
        return False
    ...
    reference = np.asarray(playback_audio, dtype=np.float32).flatten()

    # Playback is normally 24 kHz while the microphone is 16 kHz.
    if playback_rate != SAMPLE_RATE:

        new_len = max(
            1,
            int(round(len(reference) * SAMPLE_RATE / playback_rate))
        )
    ...
    for pos in range(
        start_pos,
        max(start_pos + 1, end_pos - half + 1),
        max(1, int(0.04 * SAMPLE_RATE))
    ):

        ref = reference[pos:pos + half]

        if len(ref) < half * 0.85:
            continue

        corr = _normalised_correlation(
            mic,
            ref
        )
        ...
        if best >= BARGE_ECHO_CORRELATION:
            return True

    return False


def monitor_priya_for_barge_in(audio, sample_rate, start_threshold):
    """
    Real barge-in monitor.

    The important part is WebRTC AEC3:
        microphone = user + Priya echo + room noise
        TTS WAV     = known far-end reference
        AEC3        = microphone with Priya echo removed

    We then use WebRTC's speech-probability detector on the cleaned
    signal.  The normal patient VAD is NOT changed.
    """
    ...
    if AudioProcessor is None:
        raise RuntimeError(
            "Missing pywebrtc-audio. Install it with: "
            "pip install pywebrtc-audio"
        )

    # Convert Priya's output to the microphone's 16 kHz reference rate.
    playback = np.asarray(audio, dtype=np.float32).flatten()

    if sample_rate != SAMPLE_RATE and len(playback) > 0:
        new_len = max(
            1,
            int(round(len(playback) * SAMPLE_RATE / sample_rate))
        )
        old_x = np.linspace(
            0.0, 1.0, len(playback), endpoint=False
        )
        new_x = np.linspace(
            0.0, 1.0, new_len, endpoint=False
        )
        playback_16k = np.interp(
            new_x, old_x, playback
        ).astype(np.float32)
    else:
        playback_16k = playback

    # WebRTC AEC3 + noise suppression.  No AGC here because changing
    # gain during interruption detection can make thresholds unstable.
    processor = AudioProcessor(
        ...
        ns_level=2,
        stream_delay_ms=BARGE_AEC_DELAY_MS,
    )
    ...
    speech_time = 0.0
    playback_position = 0
    playback_duration = len(playback) / float(sample_rate)
    ...
    while True:
        ...
        recent.append(chunk.copy())

        # Feed AEC in 10 ms frames.  The far-end reference is the exact
        # TTS audio currently being played.  If playback has ended, use
        # zeros for the missing reference.
        clean_parts = []
        prob_values = []
        pos = 0

        while pos < len(chunk):
            frame = chunk[pos:pos + BARGE_FRAME_SAMPLES]
            if len(frame) < BARGE_FRAME_SAMPLES:
                frame = np.pad(
                    frame,
                    (0, BARGE_FRAME_SAMPLES - len(frame))
                )

            far_start = playback_position + pos
            far_end = far_start + BARGE_FRAME_SAMPLES

            if far_start < len(playback_16k):
                far = playback_16k[far_start:far_end]
                if len(far) < BARGE_FRAME_SAMPLES:
                    far = np.pad(
                        far,
                        (0, BARGE_FRAME_SAMPLES - len(far))
                    )
            else:
                far = np.zeros(
                    BARGE_FRAME_SAMPLES,
            ...
            prob_values.append(
                float(processor.speech_probability)
            )

            pos += BARGE_FRAME_SAMPLES

        cleaned_chunk = np.concatenate(clean_parts)
        cleaned_chunk = cleaned_chunk[:len(chunk)]
        speech_probability = (
            max(prob_values) if prob_values else 0.0
        )

        clean_level = rms(cleaned_chunk)

        # Two independent gates are required:
        #   1. meaningful residual voice energy after AEC
        #   2. WebRTC spectral speech probability
        # This prevents room noise / keyboard clicks / tiny echo tails
        # from being treated as a barge-in.
        is_speech = (
            clean_level >= start_threshold
            and speech_probability >= BARGE_SPEECH_PROBABILITY
        )
        ...
        if is_speech:
            ...
            if speech_time >= BARGE_MIN_SPEECH_SECONDS:
                pending_barge_audio = list(recent)

                print()
                print(
                    "🛑 BARGE-IN detected — Voxera stopped speaking."
                )
            ...
            # Don't immediately forget a real voice because one 10 ms
            # frame dipped below the probability threshold.
            if speech_time > 0.0:
                speech_time = max(
                    0.0,
                    speech_time - 0.04
...
def play_priya():
    ...
    try:
        ...
        # Remove any microphone audio that accumulated while TTS
        # was being generated.  This prevents stale audio from
        # causing a false barge-in.
        clear_audio_queue()
        ...
        interrupted = monitor_priya_for_barge_in(
            audio,
            sample_rate,
            barge_start_threshold
        )

        if interrupted:

            print(
                "🎤 Listening to the interrupted patient turn..."
            )

            return "barge_in"

        # Ensure playback has completely finished before returning.
        sd.wait()

        # Anything captured after playback ended is stale for the
        # next patient turn.
        clear_audio_queue()
...
def main():
    ...
    try:
        ...
        with sd.InputStream(
            ...
            # The barge-in detector uses the same calibrated start
            # threshold as the normal patient VAD, capped by the
            # existing baseline threshold limits.
            global barge_start_threshold
            barge_start_threshold = start_threshold
            ...
            while True:
                ...
                # --------------------------------------------
                # If Priya was interrupted, continue recording the
                # SAME patient turn.  Otherwise wait normally.
                # --------------------------------------------
