...
#     ↓
# Supabase persistent conversation memory
#     ↓
# Priya / Kokoro
#     ↓
# Speaker
...
except Exception as _supabase_import_error:
    SUPABASE_AVAILABLE = False
    _supabase_import_error = str(_supabase_import_error)
...
# Supabase call memory.  The microphone demo does not have a real PSTN
# caller number yet, so a configurable demo patient is used.  When the
# phone backend is connected, it should supply the real patient identity.
supabase_patient = None
supabase_call = None
supabase_call_id = None
supabase_memory_enabled = False
...
Read the patient's current message AND the recent conversation context.
Determine whether the current message is about a health, medical, physical,
mental wellbeing, injury, symptom, illness, medication, treatment, emergency,
healthcare, or caregiving concern.

A follow-up question that is understandable only from the previous healthcare
conversation is still HEALTHCARE. Do not classify a message as OUT_OF_SCOPE
just because the current message is short, vague, or refers to something
mentioned earlier.
...
def classify_scope(
    ...
    context_block = conversation_context.strip()

    if context_block:
        user_content = (
            "RECENT HEALTHCARE CONVERSATION:\n"
            + context_block
...
def build_healthcare_messages(
    ...
    # Prefer the persistent call history fetched from Supabase.
    # Fall back to the local in-memory conversation if Supabase is unavailable.
    recent = []

    if conversation_context:
        # The context is already formatted as spoken turns.  Keep it as a
        # single user-side context block so old turns cannot accidentally
        # become new instructions to the model.
        messages.append({
            "role": "user",
            "content": (
...
# ============================================================
# SUPABASE CALL MEMORY
# ============================================================

def initialize_supabase_memory():
    """Create the demo patient/call used by the local microphone prototype."""
    global supabase_patient
    global supabase_call
    global supabase_call_id
    global supabase_memory_enabled

    if not SUPABASE_AVAILABLE:
        print("⚠️ Supabase module unavailable; using local conversation memory.")
        return False

    demo_phone = os.getenv("VOXERA_DEMO_PHONE", "9999999999").strip()
    demo_name = os.getenv("VOXERA_DEMO_PATIENT_NAME", "Voxera Demo Patient").strip()
    demo_language = os.getenv("VOXERA_DEMO_LANGUAGE", "English").strip() or "English"

    print("💾 Starting Supabase conversation memory...")

    try:
        supabase_patient = get_or_create_patient(
            full_name=demo_name,
            phone=demo_phone,
            preferred_language=demo_language,
        )

        supabase_call = create_call(
            language=demo_language,
            call_type="initial_assessment",
        )

        supabase_call_id = supabase_call["id"]
        supabase_memory_enabled = True

        print(f"   Patient ID: {supabase_patient['id']}")
        print(f"   Call ID: {supabase_call_id}")
        print("✅ Supabase memory ready.")
        print()
        return True

    except Exception as e:
        supabase_memory_enabled = False
        print(f"❌ Supabase memory initialization failed: {e}")
        print("⚠️ Continuing with local conversation memory only.")
        print()
        return False


def get_persistent_conversation_context(limit=8):
    """Fetch recent spoken turns from Supabase and format them for the LLM."""
    if not supabase_memory_enabled or not supabase_call_id:
        return ""

    try:
        turns = get_recent_conversation(
            supabase_call_id,
            limit=limit,
        )
        ...
        for turn in turns:
            ...
            if speaker == "patient":
                ...
                label = speaker.upper() or "UNKNOWN"
        ...
        print(f"⚠️ Could not fetch Supabase conversation: {e}")
        return ""


def save_turn_to_supabase(speaker, message):
    """Persist one spoken patient/AI turn without breaking the call on DB errors."""
    if not supabase_memory_enabled or not supabase_call_id or not message:
        return False
    ...
    try:
        ...
        print(f"⚠️ Supabase turn save failed: {e}")
        return False


def finish_supabase_call(outcome="completed"):
    """Mark the current demo call as completed when the process exits."""
    if not supabase_memory_enabled or not supabase_call_id:
        return
    ...
    try:
        ...
        print("💾 Supabase call marked completed.")
    except Exception as e:
        print(f"⚠️ Could not update Supabase call status: {e}")
...
def save_conversation(
    ...
    # --------------------------------------------------------
    # Store ONLY actual spoken content locally.
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # Persist the same spoken turns in Supabase.
    # --------------------------------------------------------

    if patient_text:
        save_turn_to_supabase("patient", patient_text)

    if response:
        save_turn_to_supabase("ai", response)

    # --------------------------------------------------------
    # Keep local memory tiny.
    # --------------------------------------------------------
...
def process_turn(
    ...
    # --------------------------------------------------------
    # FETCH PREVIOUS CALL CONTEXT.
    # --------------------------------------------------------
    # Fetch BEFORE saving the current turn so the current message is not
    # duplicated inside the context block. It is passed separately to both
    # AI stages below.
    persistent_context = get_persistent_conversation_context(limit=8)

    # --------------------------------------------------------
    # PERSIST THE CURRENT PATIENT TURN.
    # --------------------------------------------------------
    # The complete spoken transcript is still written to Supabase before
    ...
    if emergency:
        ...
        # Add the response locally and persist ONLY the AI turn here,
        # because the patient turn was already saved above.
        conversation.append({
            "role": "user",
            "content": patient_text
        })
    ...
    print(
        "🔎 Checking healthcare scope with conversation context..."
    )
...
def main():
    ...
    try:
        ...
        finish_supabase_call("completed")
